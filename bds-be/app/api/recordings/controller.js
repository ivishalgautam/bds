...
const create = async (req, res) => {
    try {
        const teacher_id = (
            await table.TeacherModel.getByUserId(req.user_data.id)
        ).id;
        if (!teacher_id) {
            return res.send(
...
const update = async (req, res) => {
    try {
        ...
        if (!record) {
            return res.send(
                "not_found",
                "recordings not found or recordings is deleted"
            );
            return;
        }
...
const deleteById = async (req, res) => {
    try {
        ...
        if (record === 0) {
            return res.send(
                "not_found",
                "recording not found or product is deleted"
            );
            return;
        }
        return res.send( {
...
const get = async (req, res) => {
    try {
        if (req.user_data.role === "teacher") {
            ...
            if (!teacher) {
                return res.send( "not_found", "teacher not exists");
                return;
            }
        ...
        if (req.user_data.role === "student") {
            const student = await table.StudentModel.getByUserId(
                req.user_data.id
            );
